export default async function handler(req, res) {
  ...
  try {
    ...
    // Buscar el evangelio - puede tener diferentes encabezados
    const markers = [
      'Evangelio del día',
    ...
    let text = null;
    if (gospelStart > -1) {
      text = rawText.substring(gospelStart, gospelEnd > -1 ? gospelEnd : gospelStart + 3000).trim();
    }

    if (!text || text.length < 100) {
      return res.status(404).json({ success: false, error: 'No se encontró el evangelio', marker: markerUsed });
    }
