...
Sistema reutilizable para marcar cualquier sección como "nueva" hasta que el usuario la visite — pensado para no escribir lógica nueva cada vez que se lance algo. Registro por dispositivo en `localStorage` (`horeb_seen_features`, array de ids visitados), sin requerir sesión. Mismo punto de luz (`LightDot`) que ya usa el rastro de novedad de Conec✝2 — ningún indicador nuevo inventado.

- **Para agregar una función futura**: (1) un id único, (2) agregarlo a `NEW_FEATURE_IDS`, (3) `isNewFeature(id)` para mostrar el punto donde corresponda y `markFeatureVisited(id)` en su `onClick` — el indicador aparece y desaparece solo, sin condicionales nuevas por función.
- "Nueva" se define como "el usuario nunca la visitó" (`!seenFeatures.has(id)`), nunca por fecha de lanzamiento — así, cualquier usuario que ya usaba la app antes de agregar una función también la ve marcada, sin lógica especial para "usuarios existentes".
- Se apaga al abrir la sección (mismo criterio que Conec✝2: "se apaga al abrir, no por ignorarlo"), sin importar si el usuario completa la acción principal.
- **Aplicado a Lectio Divina**: `featureId: 'lectio-divina'` en su tarjeta de Oración Personal; `NEW_FEATURE_IDS = ['lectio-divina']`.
- Verificado con Playwright, simulando un dispositivo que nunca usó la función (sin la clave `horeb_seen_features` en `localStorage`, el caso real de cualquier usuario existente desde antes de ayer): el punto aparece en la tarjeta; al tocarla, `localStorage` pasa a `["lectio-divina"]` y el punto desaparece; tras recargar la página por completo, el punto sigue sin aparecer (persistencia real, no solo en memoria). No se tocó `sw.js`.