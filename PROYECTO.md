...
- **Reconstrucción con fuente completa (23 jul 2026, commit `be7db30`) — pendiente de aprobación de Carlos, no dar por cerrado.** La versión anterior estaba incompleta en dos puntos: solo una década (no cinco) y una versión corta de las oraciones de apertura. Estructura correcta ahora: Señal de la Cruz → Oración de Santa Faustina (Diario, 72, en 2 pantallas: la oración + su continuación "Expiraste, Jesús...") → Oh Sangre y Agua ×3 → Padre Nuestro/Ave María/Credo → **5 décadas** (Grano Mayor + 10 Granos Menores cada una, mismo anillo y mecanismo que el Rosario, insignia "Década X de 5") → Santo Dios ×3 → **2 oraciones finales** (ambas siempre visibles, ninguna opcional — la segunda es Diario, 1570) → cierre
- **Bug real encontrado y resuelto (25 jul 2026): saltaba décadas en dispositivo táctil real.** Carlos lo confirmó en celular después de descartar caché (desinstaló/reinstaló limpio). No se pudo reproducir con clics de script ni con toques manuales espaciados — solo aparecía en toque real. Causa raíz: `handleBeadTap` (idéntico a `handleAveMariaTap` de `Rosario.jsx` — **ninguno de los dos tenía protección**) lee `counts` del closure en vez de usar la forma funcional; si el mismo toque físico dispara el evento dos veces (touchstart + click sintetizados, típico en móvil), ambos disparos leen el mismo valor viejo antes de que React re-renderice, y en la última cuenta de una década cada disparo duplicado programa su propio `setTimeout(goNext, 400)` — como `goNext` sí usa forma funcional, los dos se acumulan y saltan una página de más por cada duplicado. Con 5 décadas repitiendo el mismo patrón (mucha más oportunidad de duplicado que en cualquier otro flujo), los saltos se acumulan hasta aterrizar muy adelante. **Arreglado con `advancingRef`** (un ref, no estado — se actualiza al instante, así el segundo disparo del mismo toque ya lo ve bloqueado). Probado con un caso de reproducción real: dos eventos `click()` disparados sin espera en el mismo botón — sin el fix salta la página "Grano Mayor" completa; con el fix aterriza exactamente donde debe
- **Mismo fix aplicado a `Rosario.jsx` (25 jul 2026):** `handleAveMariaTap` tenía la vulnerabilidad idéntica — mismo `advancingRef`. Confirmado con la misma prueba de antes/después (doble `click()` sin espera en la 10ª Ave María): sin el fix saltaba "Gloria" y aterrizaba en "Jaculatoria"; con el fix aterriza correctamente en "Gloria". Persistencia y retomar del Rosario verificados sin cambios de comportamiento — recarga a mitad de un misterio (6/10 Ave Marías) retoma con el contador en 6, no reiniciado, y el mensaje sigue siendo específico ("Dejaste el Rosario en el Misterio 1: ...")
- **Service worker bumpeado a v19 (25 jul 2026)** — este fix afecta a cualquier usuario con celular, en ambas devociones, así que se subió la versión para que llegue rápido a quien tenga la app cacheada
...
- `public/sw.js` (v19 — corregido 25 jul 2026: el commit `1b20f0c` decía "sw v19" en el mensaje pero nunca tocó el archivo; seguía en v18 hasta el fix del doble disparo de esta sesión), `public/favicon.svg`, `public/manifest.json`, `generate-icons.js`
...
- Service worker en v19 (bump real del 25 jul 2026 — el v19 anterior solo existía en el mensaje del commit, nunca en el archivo)
